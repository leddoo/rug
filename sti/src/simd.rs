...
pub use core::simd::{LaneCount, SupportedLaneCount, SimdPartialOrd, SimdOrd, SimdPartialEq, SimdFloat, SimdInt};
...
impl<const N: usize> NumExt for U32x<N> where LaneCount<N>: SupportedLaneCount {
    ...
    const MIN: Self  = Self::from_array([u32::MIN; N]);
    const MAX: Self  = Self::from_array([u32::MAX; N]);
}

impl<const N: usize> NumExt for I32x<N> where LaneCount<N>: SupportedLaneCount {
    const ZERO: Self = Self::from_array([0; N]);
    const ONE: Self  = Self::from_array([1; N]);
    const MIN: Self  = Self::from_array([i32::MIN; N]);
    const MAX: Self  = Self::from_array([i32::MAX; N]);
}

impl<const N: usize> NumExt for F32x<N> where LaneCount<N>: SupportedLaneCount {
    const ZERO: Self = Self::from_array([0.0; N]);
    const ONE: Self  = Self::from_array([1.0; N]);
    const MIN: Self  = Self::from_array([f32::MIN; N]);
    const MAX: Self  = Self::from_array([f32::MAX; N]);
}
